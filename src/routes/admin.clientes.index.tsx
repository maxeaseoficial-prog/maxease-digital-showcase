    ...
    <div>
      ...
      {data.clients.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center">
          ...
          <button type="button" onClick={() => setOpen(true)}
            className="mt-5 inline-flex items-center gap-2 h-9 rounded-lg bg-slate-900 px-4 text-[13.5px] font-medium text-white transition-colors hover:bg-slate-800">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.clients.map((c) => (
            <div key={c.id} className="group rounded-2xl border border-slate-200/80 bg-white p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_10px_30px_-18px_rgba(15,23,42,0.25)]">
              <div className="flex items-start justify-between">
                <div className="h-11 w-11 rounded-xl bg-slate-100 text-slate-500 flex items-center justify-center font-semibold overflow-hidden ring-1 ring-slate-200">
...
function NewClientModal({ onClose, onCreate }: { onClose: () => void; onCreate: (input: { email: string; password: string; name: string; company: string; activeProject: string; avatarUrl?: string }) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-white rounded-2xl border border-slate-200/80 shadow-[0_30px_80px_-30px_rgba(15,23,42,0.35)]">
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          <div className="flex items-center gap-4">
            <div className="h-16 w-16 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center overflow-hidden text-xl font-semibold shrink-0 ring-1 ring-slate-200">
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            <button type="submit" className="inline-flex items-center gap-2 h-9 px-4 rounded-lg bg-slate-900 text-[13.5px] text-white font-medium transition-colors hover:bg-slate-800">
              <Plus className="h-4 w-4" /> Criar cliente
            </button>
          </div>
        </form>
...
function ConfirmDialog({ title, message, confirmLabel, onCancel, onConfirm }: { title: string; message: string; confirmLabel: string; onCancel: () => void; onConfirm: () => void }) {
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={onCancel}>
      <div className="w-full max-w-md bg-white rounded-2xl border border-slate-200/80 shadow-[0_30px_80px_-30px_rgba(15,23,42,0.35)] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="p-6">
